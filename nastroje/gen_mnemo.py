#!/usr/bin/env python3
"""Sestaví z `farmakologie_mnemo.py` mnemotechnický atlas (.md → .pdf).

    python3 nastroje/gen_mnemo.py

Jedna otázka = jedna strana = jeden zapamatovatelný obraz. Výstup:
Projekty/Studium/Predmety/Farmakologie/minimum/MNEMO.{md,pdf}
"""
import re
import subprocess
import sys
...
import farmakologie_mnemo as fm  # noqa: E402
...
def main() -> None:
    casti = [f"{svg}\n{ZLOM}\n" for _, _, svg in sorted(fm.O, key=_poradi)]
    md_cesta = CIL / "MNEMO.md"
    ...
    chybi = sorted(ocekavane - {c for c, _, _ in fm.O},
                   key=lambda c: (0 if c.startswith("O") else 1, int(c.lstrip("O"))))
    print(f"{pdf.name} · {len(fm.O)} otázek · {d.page_count} stran · {len(toc)} záložek")
    if chybi:
        print("  ⚠️ chybí:", chybi)
