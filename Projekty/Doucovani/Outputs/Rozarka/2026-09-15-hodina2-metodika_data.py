# -*- coding: utf-8 -*-
# Metodický tahák k hodině 2 — jedno cvičení na stranu.
# Struktura bloku: (číslo strany listu, název, minuty, cíl, ŘÍCT, NAVÍC, GRAMATIKA, ŘEŠENÍ, LEHČÍ, TĚŽŠÍ)

CVICENI = [

("2","Hello again! + kontrola úkolu","3 min",
 "Rozmluvit ji po týdnu a zjistit, jestli si pamatuje slovíčka z minula.",
 [("How are you today?","hau ár jú tu-DEJ","Jak se dneska máš?"),
  ("What's the weather like today?","wots ď WEDR lajk tu-DEJ","Jaké je dnes počasí?"),
  ("What day is it today?","wot dej iz it tu-DEJ","Jaký je dnes den?"),
  ("Show me your homework.","šou mí jor HOUM-vérk","Ukaž mi domácí úkol."),
  ("Read me one sentence.","ríd mí wan SENTNS","Přečti mi jednu větu.")],
 [("Why?","waj","Proč? — nejlevnější způsob, jak z ní dostat druhou větu"),
  ("Really? Tell me more.","RY-ly? tel mí mór","Vážně? Řekni mi víc."),
  ("Me too!","mí tú","Já taky!"),
  ("Is it cold or hot?","iz it kould or hot","Je zima, nebo horko? — nabídni volbu, když mlčí"),
  ("And your sister?","end jor SISTR","A co tvoje sestra?"),
  ("What did you do yesterday?","wot did jú dú JESTR-dej","Co jsi dělala včera? — jen když jí to jde")],
 [("„What's the weather <b>like</b>?“ &mdash; tohle LIKE neznamená „mít rád“.",
   "Je to vazba <b>What is … like?</b> = „Jaké je …?“. Rozárka to zná jako <i>I like</i> a splete si to. "
   "Řekni jí česky: <b>„Když je like na konci otázky, ptáš se JAKÉ to je, ne jestli to máš ráda.“</b>"),
  ("Dny v týdnu se píšou <b>velkým písmenem</b>.",
   "Monday, Tuesday … &mdash; vždycky velké M, T. V češtině malé, v angličtině velké. Hlídej to, když píše.")],
 [("Klíč k úkolu","suitcase = kufr &middot; tent = stan &middot; mountains = hory &middot; "
   "castle = hrad &middot; plane = letadlo &middot; seaside = u moře")],
 "Neptej se otevřeně. Nabídni dvě možnosti: <i>„Is it Monday or Tuesday?“</i>",
 "Po každé odpovědi přidej <b>„Why?“</b> a chtěj větu s <b>because</b>."),

("3","Action words","5 min",
 "Dvanáct nových činností &mdash; aktivně, ne jen přečíst.",
 [("What can he do?","wot ken hí dú","Co umí? — u obrázku"),
  ("Say it in English.","sej it in ING-liš","Řekni to anglicky."),
  ("Repeat after me.","ri-PÍT áftr mí","Opakuj po mně."),
  ("Now write it in Czech.","nau rajt it in ček","Teď to napiš česky.")],
 [("Can you do it?","ken jú dú it","Umíš to? — propojí slovíčko s dnešní gramatikou"),
  ("Show me!","šou mí","Ukaž mi to! — ať to předvede u kamery"),
  ("Do you like it?","dú jú lajk it","Baví tě to?"),
  ("Who can do this in your family?","hú ken dú dis in jor FE-mily","Kdo to ve vaší rodině umí?"),
  ("Is it easy or difficult?","iz it ÍZY or DIFI-klt","Je to lehké, nebo těžké?"),
  ("What else can you do?","wot els ken jú dú","Co ještě umíš?")],
 [("<b>play football</b>, ale <b>play THE piano</b>.",
   "U <b>sportů</b> se člen nedává: play football, play tennis, play chess. "
   "U <b>hudebních nástrojů</b> se dává <b>the</b>: play the piano, play the guitar. "
   "Česky jí to řekni takhle: <b>„U sportu nic, u nástroje THE.“</b> Jiné pravidlo za tím není, je to prostě zvyk."),
  ("Proč <b>ride A bike</b> a ne jen <i>ride bike</i>?",
   "<i>Ride</i> = jezdit na něčem, takže potřebuje předmět a ten potřebuje člen. "
   "Angličtina skoro nikdy nenechá počitatelné jednotné číslo bez členu &mdash; <b>a bike, a horse, a car</b>.")],
 [("Výslovnost, kde chybuje nejvíc",
   "draw [dró] &mdash; W se nevyslovuje &middot; dance [dáns] &mdash; britsky dlouhé Á &middot; "
   "sing [sing] &mdash; nosové NG, ne „sink“ &middot; ski [skí] &mdash; dlouhé Í")],
 "Napověz první písmeno: <i>„It starts with S…“</i> [it stárts wid es]",
 "Chtěj celou větu: ne <i>swim</i>, ale <b>I can swim.</b>"),

("4","Word race (na čas)","4 min",
 "Rychlé vybavování a nenápadné opakování slovíček z hodiny 1.",
 [("No words &mdash; just pictures!","nou vérdz &mdash; džast PIK-črz","Žádná slova, jen obrázky!"),
  ("Ready? Go!","RE-dy? gou","Připravená? Teď!"),
  ("Time's up!","tajmz ap","Čas vypršel!"),
  ("How many?","hau ME-ny","Kolik jsi jich dala?"),
  ("Can you beat it?","ken jú bít it","Zvládneš to překonat?")],
 [("Faster!","FÁS-tr","Rychleji!"),
  ("Don't stop!","dount stop","Nezastavuj se!"),
  ("Skip it!","skip it","Přeskoč to! — když se zasekne, jde se dál"),
  ("That's better!","dets BE-tr","To je lepší!"),
  ("One more round?","wan mór raund","Ještě jedno kolo?"),
  ("That's your record!","dets jor RE-kord","To je tvůj rekord!")],
 [("Tady se gramatika neřeší &mdash; a to je záměr.",
   "Cílem je rychlost vybavení, ne správná věta. Když řekne holé <i>swim</i> místo <i>I can swim</i>, "
   "je to v pořádku. <b>Neopravuj během kola</b>, zabiješ tím tempo i zážitek.")],
 [("Klíč &mdash; 20 obrázků po řadách",
   "<b>1.</b> swim &middot; ride a bike &middot; run &middot; sing &middot; dance<br>"
   "<b>2.</b> cook &middot; draw &middot; play football &middot; play tennis &middot; play the piano<br>"
   "<b>3.</b> ski &middot; skate &middot; beach &middot; sea &middot; ice cream<br>"
   "<b>4.</b> mountains &middot; sun &middot; tent &middot; suitcase &middot; plane")],
 "Zkrať na <b>30 sekund</b> a počítej jen první dvě řady.",
 "Ať říká celé věty <b>I can / I can't</b> &mdash; za minutu jich dá míň, ale je to těžší."),

("5","I can / I can't","5 min",
 "Říct pravdu o sobě celou větou. Tady se láme dnešní gramatika.",
 [("Circle can or can't.","SÉR-kl ken or kánt","Zakroužkuj can nebo can't."),
  ("True for you!","trú for jú","Podle pravdy, jak to máš ty!"),
  ("Now write three sentences.","nau rajt thrí SENTN-siz","Teď napiš tři věty."),
  ("Read them out loud.","ríd dem aut laud","Přečti je nahlas.")],
 [("Are you good at it?","ár jú gud et it","Jde ti to?"),
  ("A little bit.","e litl bit","Trochu. — užitečná odpověď mezi ano a ne"),
  ("Can your mum do it?","ken jor mam dú it","Umí to tvoje máma? — most ke třetí osobě"),
  ("What about your dad?","wot e-BAUT jor ded","A co táta?"),
  ("Say it with BUT.","sej it wid bat","Řekni to se slovem ALE."),
  ("I can swim but I can't ski.","aj ken swim bat aj kánt skí","Umím plavat, ale neumím lyžovat.")],
 [("<b>Can se nikdy nemění. Ani u she, ani u my mum.</b>",
   "Can je <b>modální sloveso</b> a ta se nečasují &mdash; nikdy nedostanou koncové -s. "
   "<i>I can, you can, she can, my mum can, they can.</i> Proto je <b>My mum cans swim</b> chyba."),
  ("Po <b>can</b> jde holé sloveso.",
   "Žádné <i>to</i>, žádné <i>-ing</i>, žádné <i>-s</i>: <b>I can swim</b> &mdash; ne <i>I can to swim</i>, "
   "ne <i>I can swimming</i>."),
  ("Jak to vysvětlit Rozárce česky",
   "<b>„Can je jako české UMÍM. Řekneš ‚umím plavat‘ &mdash; a to sloveso za tím zůstane holé. "
   "Nic se k němu nelepí, ať mluvíš o sobě, nebo o mámě.“</b>"),
  ("Proč to plete &mdash; a proč to není její chyba",
   "Minulou hodinu se učila pravý opak: <i>My mum <b>likeS</b> ice cream</i>. Tam se -s přidávat MUSELO. "
   "Teď se nesmí. <b>Tohle rozlišení je celý smysl dnešní hodiny</b>, ne vedlejší detail &mdash; "
   "počítej s tím, že to splete, a neber to jako nepozornost.")],
 [("Vzorové věty","I can draw. [aj ken dró] &middot; I can't ski. [aj kánt skí] &middot; "
   "My mum can cook. [máj mam ken kuk]")],
 "Ukazuj na obrázky a ptej se <i>„Can you swim?“</i> &mdash; ať jen odpovídá, psaní nech na konec.",
 "Přidej třetí osobu: <b>My mum can …</b> a <b>My dog can …</b>"),

("6","Can you…? (ptá se ONA tebe)","5 min",
 "Vytvořit otázku. Nejdůležitějších pět minut hodiny.",
 [("Now you ask ME!","nau jú ásk mí","Teď se ptáš ty mě!"),
  ("Yes, I can.","jes, aj ken","Ano, umím."),
  ("No, I can't.","nou, aj kánt","Ne, neumím."),
  ("A little bit.","e litl bit","Trochu."),
  ("Now tell me about me!","nau tel mí e-BAUT mí","Teď mi řekni něco o mně!")],
 [("Ask me another one.","ásk mí e-NA-dr wan","Zeptej se mě na další."),
  ("Good question!","gud KWES-čn","Dobrá otázka!"),
  ("Guess!","ges","Hádej!"),
  ("Why do you ask?","waj dú jú ásk","Proč se ptáš?"),
  ("Ask your mum tonight!","ásk jor mam tu-NAJT","Zeptej se večer mámy! — úkol navíc"),
  ("Now ask about my dog.","nau ásk e-BAUT máj dog","Teď se zeptej na mého psa.")],
 [("Otázka s can: <b>jen se prohodí pořadí</b>.",
   "<i>You can swim.</i> &rarr; <b>Can you swim?</b> Can jde dopředu, nic se nepřidává."),
  ("⚠️ U can se <b>nikdy</b> nepoužívá DO.",
   "<i>Do you can swim?</i> je chyba &mdash; a udělá ji, protože u obyčejných sloves se DO přidávat MUSÍ: "
   "<i>Do you like ice cream?</i> Rozdíl: <b>obyčejné sloveso potřebuje pomocníka DO, can si pomůže samo.</b>"),
  ("Krátká odpověď sloveso neopakuje.",
   "<b>Yes, I can.</b> &mdash; ne <i>Yes, I can swim.</i> Není to chyba, ale zní to nepřirozeně."),
  ("Jak to vysvětlit česky",
   "<b>„V češtině taky nic nepřidáváš: ‚ty umíš‘ &rarr; ‚umíš ty?‘. Jenom to prohodíš. "
   "S can je to úplně stejné.“</b>")],
 [("Co má na konci napsat","My teacher can cook. [máj tíčr ken kuk]<br>"
   "My teacher can't ski. [máj tíčr kánt skí]<br>"
   "<b>Pozor na cans</b> &mdash; když ho napíše, ukaž rámeček dole na téže straně listu.")],
 "Nech ji otázky <b>číst</b> z listu. Už jen vyslovit je práce.",
 "Zakryj otázky a ať je vymyslí sama: <i>„Ask me about sport.“</i>"),

("7","Secret action (hra)","5 min",
 "Otázky a krátké odpovědi v opravdové komunikaci, ne ve cvičení.",
 [("Circle one &mdash; don't tell me!","SÉR-kl wan &mdash; dount tel mí","Jednu zakroužkuj &mdash; neříkej mi to!"),
  ("Can you ride a bike?","ken jú rajd e bajk","Umíš jezdit na kole?"),
  ("I know! You can cook!","aj nou! jú ken kuk","Já vím! Ty umíš vařit!"),
  ("My turn! / Your turn!","máj térn / jor térn","Já jsem na řadě! / Ty jsi na řadě!")],
 [("Is it a sport?","iz it e sport","Je to sport?"),
  ("Do you need a ball?","dú jú níd e ból","Potřebuješ k tomu míč?"),
  ("Inside or outside?","in-SAJD or aut-SAJD","Uvnitř, nebo venku?"),
  ("Hmm, let me think.","hm, let mí think","Hmm, nech mě přemýšlet."),
  ("Last question!","lást KWES-čn","Poslední otázka!"),
  ("You got me!","jú got mí","Dostalas mě!")],
 [("Všimni si rozdílu v těch dvou otázkách &mdash; a ukaž jí ho.",
   "<b>Can</b> you ride a bike? &mdash; prohodí se.<br>"
   "<b>Do</b> you need a ball? &mdash; přidá se DO.<br>"
   "Je to přesně ten rozdíl ze strany 6 listu, tentokrát v běžné řeči. Když se zeptá "
   "<i>„Do you can…?“</i>, zopakuj otázku správně a jeď dál."),
  ("Druhá polovina hry je ta cennější.",
   "Dokud odpovídá, jen opakuje dvě naučené věty. Teprve když <b>ptá se ona</b>, musí otázku vyrobit. "
   "Nech si na výměnu rolí aspoň dvě minuty.")],
 [("Jak to hrát","Ona si tajně zakroužkuje jednu z dvanácti činností, ty hádáš. "
   "Za každou otázku odškrtne kolečko &mdash; máš deset. Odpovídá jen <b>Yes, I can</b> / <b>No, I can't</b>.")],
 "Zúži výběr: <i>„Only the first row.“</i> [ounly ď férst rou] &mdash; jen první řada.",
 "Dej si limit <b>pět otázek</b> a hraj o body."),

("8+9","Homework a bonus","3 min + rezerva",
 "Odejít s jasným zadáním. A mít co dělat, kdyby zbyl čas.",
 [("This is your homework.","dis iz jor HOUM-vérk","Tohle je tvůj domácí úkol."),
  ("Learn these six words.","lérn dýz siks vérdz","Nauč se těchhle šest slovíček."),
  ("Draw yourself.","dró jor-SELF","Nakresli sama sebe."),
  ("Can you tell me the homework?","ken jú tel mí ď HOUM-vérk","Můžeš mi říct, co máš za úkol?")],
 [("Don't forget!","dount for-GET","Nezapomeň!"),
  ("Show me next week!","šou mí nekst wík","Ukážeš mi to příští týden!"),
  ("Choose a square.","čúz e skwér","Vyber si políčko. (piškvorky)"),
  ("Three in a row &mdash; you win!","thrí in e rou &mdash; jú win","Tři v řadě &mdash; vyhrálas!"),
  ("Bad luck!","bed lak","Smůla!"),
  ("Rematch?","RÍ-meč","Odvetu?")],
 [("Třetí věta úkolu je schválně ve třetí osobě.",
   "<b>My best friend can …</b> &mdash; aby si doma znovu ohlídala, že se <b>can</b> nemění. "
   "Když to zvládne v úkolu sama, gramatiku má."),
  ("Piškvorky nejsou odměna zadarmo.",
   "Políčko dostane <b>jen za vyslovenou větu</b> s can. Hraj naostro &mdash; když ji necháš vyhrát, "
   "hra ztratí smysl a ona to pozná.")],
 [("Vzorové věty do úkolu","I can ride a bike. [aj ken rajd e bajk]<br>"
   "I can't skate. [aj kánt skejt]<br>"
   "My best friend can sing. [máj best frend ken sing]")],
 "Úkol zkrať na <b>tři slovíčka a jednu větu</b>. Lepší málo a hotové.",
 "Přidej: ať napíše <b>jednu větu o kamarádovi</b> a jednu <b>zápornou</b>."),
]

# ---------- CO ŘEKNE ONA ----------
# klíč = číslo strany listu; (modelový dialog, časté chyby)
# dialog: (kdo, text, výslovnost, česky)   kdo: "TY" / "ONA"
# chyby:  (co řekne, správně, proč)

DIALOGY = {

"2": ([
 ("TY","How are you today?","hau ár jú tu-DEJ",""),
 ("ONA","I'm fine, thank you.","ajm fajn, thenk jú","Mám se dobře, děkuju."),
 ("TY","What's the weather like today?","wots ď WEDR lajk tu-DEJ",""),
 ("ONA","It's sunny. / It's raining.","its SA-ny / its REJ-ning","Je slunečno. / Prší."),
 ("TY","What day is it today?","wot dej iz it tu-DEJ",""),
 ("ONA","It's Tuesday.","its TJÚZ-dej","Je úterý."),
 ("TY","Read me one sentence.","ríd mí wan SENTNS",""),
 ("ONA","In the summer I swim in the sea.","in ď SA-mr aj swim in ď sí","V létě plavu v moři."),
],[
 ("<u>Is</u> sunny.","<b>It's</b> sunny.",
  "Vynechá <b>it</b> &mdash; čeština podmět vynechává („prší“), angličtina nikdy. <b>Nejčastější česká chyba vůbec.</b>"),
 ("Fine.","<b>I'm</b> fine.","Jednoslovná odpověď. Řekni <i>„Full sentence, please.“</i> a počkej."),
 ("Today is <u>tuesday</u>.","Today is <b>T</b>uesday.","Dny se píšou velkým písmenem. (Mluvenou verzi ber, opravuj jen psanou.)"),
]),

"3": ([
 ("TY","What can he do?","wot ken hí dú",""),
 ("ONA","He can swim.","hí ken swim","Umí plavat."),
 ("TY","And this one?","end dis wan",""),
 ("ONA","He can play the piano.","hí ken plej ď pi-E-nou","Umí hrát na klavír."),
 ("TY","Can you do it?","ken jú dú it",""),
 ("ONA","Yes, I can! / No, I can't.","jes aj ken / nou aj kánt","Ano, umím! / Ne, neumím."),
 ("TY","Now write it in Czech.","nau rajt it in ček",""),
 ("ONA","(píše „plavat“)","","Stačí jedno slovo, celou větu tu nechtěj."),
],[
 ("He can <u>swims</u>.","He can swim.","Přidá -s po can. Dnešní hlavní chyba &mdash; čekej ji."),
 ("He can play <u>piano</u>.","He can play <b>the</b> piano.","U nástroje chybí <b>the</b>."),
 ("He can <u>bike</u>.","He can <b>ride a</b> bike.","<i>Bike</i> je věc, ne činnost. Potřebuje <b>ride a</b>."),
]),

"4": ([
 ("TY","Ready? Go!","RE-dy? gou",""),
 ("ONA","Swim… bike… run… sing…","","Holá slova stačí, celé věty tady nechtěj."),
 ("ONA","(zasekne se)","","Ty hned: <b>Skip it!</b> [skip it] &mdash; a ukaž na další."),
 ("TY","Time's up! How many?","tajmz ap! hau ME-ny",""),
 ("ONA","Twelve!","twelv","Dvanáct!"),
 ("TY","Can you beat it?","ken jú bít it",""),
],[
 ("„plavat“ (česky)","<b>Swim.</b>","Řekni jen <i>„In English!“</i> [in ING-liš] a ukazuj dál. Nezastavuj se."),
 ("Mlčí a přemýšlí","&mdash;","Tři vteřiny ticha = přeskoč. Běží čas, o to jde."),
]),

"5": ([
 ("TY","Can you swim? Circle it.","ken jú swim? SÉR-kl it",""),
 ("ONA","Yes, I can.","jes, aj ken","Ano, umím."),
 ("TY","Now write three sentences.","nau rajt thrí SENTN-siz",""),
 ("ONA","I can draw.","aj ken dró","Umím kreslit."),
 ("ONA","I can't ski.","aj kánt skí","Neumím lyžovat."),
 ("ONA","My mum can cook.","máj mam ken kuk","Máma umí vařit."),
 ("TY","Are you good at it?","ár jú gud et it",""),
 ("ONA","A little bit.","e litl bit","Trochu."),
],[
 ("My mum <u>cans</u> cook.","My mum <b>can</b> cook.","Ta hlavní. Neopravuj slovy &mdash; ukaž rámeček dole na straně 6 listu."),
 ("I can <u>to</u> draw.","I can draw.","Po can jde holé sloveso."),
 ("I <u>no can</u> ski.","I <b>can't</b> ski.","Zápor dělá can't, ne přidané <i>no</i>."),
]),

"6": ([
 ("ONA","Can you cook?","ken jú kuk","Umíš vařit?"),
 ("TY","Yes, I can.","jes, aj ken",""),
 ("ONA","Can you ski?","ken jú skí","Umíš lyžovat?"),
 ("TY","No, I can't.","nou, aj kánt",""),
 ("ONA","Can you play the piano?","ken jú plej ď pi-E-nou","Umíš hrát na klavír?"),
 ("TY","A little bit.","e litl bit",""),
 ("TY","Now tell me about me!","nau tel mí e-BAUT mí",""),
 ("ONA","My teacher can cook.","máj tíčr ken kuk","Moje učitelka umí vařit."),
],[
 ("<u>Do</u> you can cook?","<b>Can</b> you cook?","Přidá DO. Zopakuj otázku správně a nech ji ji říct po tobě."),
 ("You can cook?","<b>Can you</b> cook?","Neprohodí pořadí. V mluvené řeči by to prošlo, ale teď to opravuj."),
 ("My teacher <u>cans</u> cook.","My teacher <b>can</b> cook.","Zase to -s. Ukaž rámeček dole na straně 6 listu."),
]),

"7": ([
 ("TY","Can you ride a bike?","ken jú rajd e bajk",""),
 ("ONA","No, I can't.","nou, aj kánt","Ne, neumím."),
 ("TY","Can you cook?","ken jú kuk",""),
 ("ONA","Yes, I can!","jes, aj ken","Ano, umím!"),
 ("TY","I know! You can cook!","aj nou! jú ken kuk",""),
 ("ONA","You got me!","jú got mí","Dostalas mě!"),
 ("TY","My turn is over. Your turn!","máj térn iz OU-vr. jor térn",""),
 ("ONA","Can you swim?","ken jú swim","Umíš plavat? — teď se ptá ona"),
],[
 ("„Yes.“ samotné","<b>Yes, I can.</b>","Chtěj celou krátkou odpověď. Ukaž na rámeček vedle mřížky."),
 ("„Ne.“ (česky)","<b>No, I can't.</b>","Řekni to za ni a nech ji to zopakovat."),
 ("<u>Do</u> you swim?","<b>Can</b> you swim?","Ve druhé polovině hry, když tvoří otázky sama. Čekej to."),
]),

"8+9": ([
 ("TY","Can you tell me the homework?","ken jú tel mí ď HOUM-vérk",""),
 ("ONA","Six words and three sentences.","siks vérdz end thrí SENTN-siz","Šest slovíček a tři věty."),
 ("TY","(piškvorky) Choose a square.","čúz e skwér",""),
 ("ONA","I can dance!","aj ken dáns","Umím tancovat! — a políčko je její"),
 ("TY","It's yours!","ic jorz",""),
 ("ONA","Three in a row! I win!","thrí in e rou! aj win","Tři v řadě! Vyhrála jsem!"),
],[
 ("Řekne jen „dance“","<b>I can dance.</b>","Za holé slovo políčko není. Celá věta, nebo nic &mdash; jinak hra ztratí smysl."),
 ("Neví, co má za úkol","&mdash;","Nech ji podívat se na stranu 8 listu a přečíst to nahlas. Pak to zopakuj ty."),
]),
}

# ---------- JAK NA TO: postup cvičení krok za krokem ----------
POSTUP = {

"2": [
 "Pozdrav a projeď tři otázky v levé půlce listu. Na každou chtěj <b>celou větu</b>, ne jedno slovo.",
 "Přepni na pravou půlku: <i>„Show me your homework.“</i>",
 "Nech ji šest slovíček dopsat <b>zpaměti</b>. Nenapovídej hned — dej jí pár vteřin ticha.",
 "Až to vzdá, doplňte zbytek společně podle klíče vlevo dole.",
 "<i>„Read me one sentence.“</i> — přečte jednu ze tří vět ze sešitu.",
 "Pochval a jdi dál. Na tenhle blok máš <b>tři minuty</b>, nezdržuj se.",
],

"3": [
 "Projdi všech dvanáct obrázků: ukaž na obrázek, ona řekne anglicky.",
 "Co neví, řekni ty a nech ji to zopakovat: <i>„Repeat after me.“</i>",
 "Druhé kolo <b>jen přes ta slova, která neuměla</b> — a rychleji.",
 "Teprve teď ať píše české překlady do linek. Během psaní se ptej <i>„Can you do it?“</i>",
 "Poslední minutu si ověř tři nejtěžší: <b>play the piano, ride a bike, draw</b>.",
],

"4": [
 "Vysvětli pravidla <b>česky</b>: „Ukážu obrázek, ty řekneš anglicky. Máš minutu. Co nevíš, přeskočíme.“",
 "Připrav si stopky na mobilu. <i>„Ready? Go!“</i>",
 "Ukazuj po řadách a <b>počítej správné</b>. Neopravuj, nekomentuj, nezdržuj se.",
 "<i>„Time's up! How many?“</i> — zapiš skóre do prvního políčka.",
 "Dvacet vteřin pauza a jede se druhé kolo. Pak třetí.",
 "Nejlepší výsledek zapiš do <b>žlutého rámečku</b> — příští hodinu ho bude překonávat.",
],

"5": [
 "Ukaž na první obrázek a zeptej se: <i>„Can you swim?“</i> Ona odpoví a zakroužkuje <b>can</b> nebo <b>can't</b>.",
 "Takhle projděte všech osm. <b>Rychle a ústně</b>, ještě se nepíše.",
 "Teprve pak ať napíše tři věty na linky dole.",
 "Třetí věta je <b>„My mum can …“</b> — tady se rozhodne, jestli gramatiku pochopila. Hlídej to -s.",
 "<i>„Read them out loud.“</i> — přečte je nahlas, ty opravíš výslovnost.",
],

"6": [
 "Otoč roli a řekni to <b>důrazně</b>: <i>„Now you ask ME!“</i> Je to změna a musí ji zaznamenat.",
 "Ona čte první otázku, ty odpovídáš <b>pravdivě</b> o sobě.",
 "<b>Kroužkuje ona</b>, ne ty — zakroužkuje tvoji odpověď YES nebo NO.",
 "Po pěti otázkách: <i>„Now tell me about me!“</i>",
 "Napíše dvě věty o tobě. Hlídej <b>My teacher can</b> &mdash; bez -s.",
],

"7": [
 "<i>„Circle one — don't tell me!“</i> Otoč se od kamery nebo zavři oči, ať je to opravdová hra.",
 "Ptej se <i>„Can you …?“</i> Ona odpovídá <b>Yes, I can / No, I can't</b> a odškrtává kolečka.",
 "Když uhodneš: <i>„I know! You can cook!“</i>",
 "<b>Pak výměna rolí.</b> Nech si na ni aspoň dvě minuty — je to ta cennější polovina.",
 "Ty si tajně vyber činnost, ona se ptá a odškrtává si kolečka sama.",
],

"8+9": [
 "Otevři stranu 8 listu a projděte všechny tři části úkolu.",
 "<i>„Can you tell me the homework?“</i> — ať ho <b>zopakuje vlastními slovy</b>. Tím ověříš, že rozumí.",
 "Spočítejte hvězdy na straně 1: <i>„You got seven stars today!“</i>",
 "Rozluč se anglicky: <i>„Great job! See you next week!“</i>",
 "Zbyl čas? Strana 9 — piškvorky. Políčko jen <b>za vyslovenou větu</b>.",
],
}
