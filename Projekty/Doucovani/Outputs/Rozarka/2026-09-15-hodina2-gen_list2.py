...
.yn .y{color:var(--ok,#2E8B57);border-color:#2E8B57}
...
.lbl{font-size:9.5pt;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--grey,#7A939B)}
'''
...
PLAN = [("1","Hello again!","4 min"),("2","Action words","6 min"),("3","I can / I can't","5 min"),
        ("4","Can you…?","6 min"),("5","Secret action","5 min"),("6","Homework","4 min")]

def sprite():
    parts=['<svg width="0" height="0" style="position:absolute" aria-hidden="true"><defs>']
    for k,v in reuse.items():
        parts.append('<symbol id="%s" viewBox="0 0 100 100">%s</symbol>'%(k,v))
...
def foot(n,lbl):
    return ('<div class="foot"><span>English &middot; 15 September</span>'
            '<span>%s</span><span>%d / 7</span></div>'%(lbl,n))
...
cc="".join('<div class="canc"><svg><use href="#%s"/></svg><span class="w">%s</span>'
           ...
           for i in CAN8)
H.append(f'''<div class="page"><div class="airmail"></div>
{head("3","I can / I can't","Circle <b>can</b> or <b>can't</b> for you. Then write three sentences.")}
...
H.append(f'''<div class="page"><div class="airmail"></div>
{head("4","Can you…?","Ask your teacher. Circle her answer. Then tell me about her.")}
<div class="iv">{iv}
...
H.append(f'''<div class="page"><div class="airmail"></div>
{head("5","Secret action","Circle one &mdash; don't tell! Your teacher asks, you answer.")}
...
H.append(f'''<div class="page"><div class="airmail"></div>
{head("6","Homework","See you next week!")}
<div class="hw">
...
print("list2.html hotov, stran: 7, </style> ok")
