# Speciální farmakologie II — HOTOVO

Dokončeno 2026-08-10 · **oficiální číslování** (převodní tabulka v `../okruhy.md`)

## ✅ Zpracováno všech 48 otázek Specky II

| Soubor | Otázky |
|---|---|
| `specka2-089-095.md` | **89–95** |
| `specka2-097-103.md` | **97–103** |
| `specka2-104-108.md` | **104–108** |
| `specka2-110-113.md` | **110–113** |
| `specka2-114-115.md` | **114–115** |
| `specka2-116-119.md` | **116–119** |
| `specka2-120-122.md` | **120–122** |
| `specka2-123-125.md` | **123–125** |
| `specka2-126.md` | **126** |
| `specka2-127.md` | **127** |
| `specka2-128-129.md` | **128–129** |
| `specka2-130-131.md` | **130–131** |
| `specka2-132-133.md` | **132–133** |
| `specka2-134-136.md` | **134–136** |

**Otázka 88 Makrolidy** — první otázka Specky II — je v `../vypisky-S1-53-makrolidy.md`.
⚠️ **Soubor je pojmenovaný špatně**, číslo „S1-53" neexistuje.

**PDF:** `../pdf-ustni/specka2-*.pdf` + **`SPECKA2-vse-v-jednom.pdf`** (67 stran, se záložkami)

## ⚠️ Dvě otázky nemá pokryté žádný materiál

| Otázka | Stav |
|---|---|
| **96 Antiastmatika** | v podrobné Specce 2 **není** — sehnat jinde |
| **109 Stimulancia** | v podrobné Specce 2 **není** — sehnat jinde |

Právě kvůli nim se rozchází číslování zdroje s oficiálním (posun +5 → +6 → +7).

## Rozsah po otázkách

**Cíl byl 1–1,5 strany.** Podle Tessina pravidla („pokud nejde zkrátit, zkrať co nejvíc, ale ať tam je vše podstatné") **jedenáct otázek limit překračuje** — u všech by další krácení znamenalo vyhazovat obsah, ne text:

| Otázka | Stran | Proč |
|---|---|---|
| **135** Živočišné jedy | 2,3 | ~25 druhů živočichů s toxiny a mechanismy |
| **127** Infuzní terapie | 2,2 | fakticky infuzní terapie **+** výživa v intenzivní péči |
| **136** Rtuť, arzen, olovo | 2,1 | tři kovy × kinetika, akutní i chronická otrava, terapie |
| **130** Osteoporóza | 2,1 | minerály + hormony + SERM + bisfosfonáty + výzkum |
| **126** Kůže a sliznice | 1,9 | ⚠️ **zdroj sám uvádí, že to byly původně dvě otázky** |
| **115** Hormony hypothalamu a hypofýzy | 1,9 | 14 hormonů ve třech oddílech |
| **128** Vitaminy v tucích | 1,8 | A, D, E, K — každý s vlastní farmakologií |
| **132** Obecná toxikologie | 1,8 | |
| **106** Ethylalkohol, methylalkohol | 1,6 | dvě látky + akutní i chronické účinky |
| **116** Štítná žláza | 1,6 | + příštítná tělíska a kalcitonin |
| **123** Cytostatika | 1,6 | šest mechanismů + orgánová toxicita |
| **133** Terapie otrav | 1,6 | tabulka antidot + kyanidy |

**Ostatních 36 otázek je v limitu**, většina kolem 1,0–1,3 strany.

## Chyby a zvláštnosti nalezené ve zdroji

- ⚠️ **Číslování zdroje není oficiální** — mapuj podle **názvu otázky**, ne podle čísla
- **Otázka 112** uvádí koncovky monoklonálních protilátek jako `-mumab / -momab / -ximab / -zimab`, zatímco obecná otázka 35 jako `-o- / -xi- / -zu- / -u-`. **Je to totéž dělení podle podílu myší složky**; procenta (chimérické max 40 %, humanizované 10 %) uvádí jen tenhle zdroj.
- Zdroj obsahuje oddíl **„125. Alternativní metody léčby"** (homeopatie), který **v oficiálním seznamu samostatná otázka není** — patrně dodatek k fytoterapii (131).
