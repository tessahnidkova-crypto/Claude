...
> ⚠️ **Ve zdroji jsou překlepy v odborných termínech.** U O1 to byla „oficiální/obsolentní" místo **oficinální/obsoletní**. Latinské názvy jsem proto kontroloval, ne jen přepisoval — kde se rozcházím se zdrojem, je to poznamenané.
...
| Forma | Podstata |
|---|---|
| **Roztoky** (*solutiones*) | jednofázové homogenní; **solutum** (rozpuštěná látka) + **solvens** (rozpouštědlo) |
| **Kapky** (*guttae*) | menší objem, vyšší koncentrace |
| **Sirupy** (*sirupi*) | vodné roztoky s velkým podílem sacharózy; nestabilní v teple a na světle → chlad a tmavé sklo |
...
Tuhé mechanicky pevné výlisky z tabletoviny. **Ne všechny lze půlit** — jen ty s **půlicí rýhou**.

| Latinsky | Česky | Podstata |
|---|---|---|
| *tabulettae non obductae* | neobalené | konvenční, „1. generace"; rychlé masivní uvolnění, rychlý ale krátký účinek (paralen, acylpyrin) |
| *tabulettae obductae* | obalované, dražé | jádro + obal; funkce **estetická** (chuť, polykání), **technologická** (ochrana), **terapeutická** (targeting) |
| *tabulettae enterosolventes* | enterosolventní | **acidorezistentní** — odolají žaludku, uvolní až ve střevě; obal se rozpadá při vyšším pH |
| *tabulettae effervescentes* | šumivé | kyselina + uhličitan → CO₂; **chránit před vzdušnou vlhkostí** |
| *tabulettae pro dispersione* | pro disperzi | rozpustit před užitím |
| — | dispergovatelné v ústech | rozpustí se ve slinách bez zapití |

**Tobolky** (*capsulae*): **tvrdé** (*durae*) — tělo + čepička, náplní prášek nebo pelety · **měkké** (*molles*) — náplní kapaliny a oleje nepříjemné chuti, malé se nazývají **perle**. ⚠️ Otevřít a vysypat **jen podle SPC**.

**Prášky a granuláty:** jednodávkové (dělené, v sáčcích) × vícedávkové (nedělené, s odměrkou). Granuláty existují i jako šumivé, obalené, enterosolventní a s řízeným uvolňováním.
...
|---|---|---|
| **Lokální** | kloktadla (*gargarismata*), ústní vody, gely a pasty na dásně, pastilky | infekce dutiny ústní, **xerostomie u onkologických pacientů**; antiseptika, ATB, antimykotika, lokální anestetika, adstringencia |
...
> Pět požadavků na parenterália → cesty podání a jejich kompromis → injekce vs infuze → depotní formy a řada rychlosti uvolňování → dermatologika a pravidlo „mokré na mokré, suché na suché"
...
|---|---|---|
| **i.v.** | rychlý nástup, lze podat i **dráždivé** látky (cévní stěna je relativně necitlivá) | infekce při kanyle (**max 4 dny**); **nelze** podat suspenze a emulze s kapénkami nad 0,5 µm, léčiva srážející krev nebo hemolyzující, látky vychytávající ionty (tetracyklin) |
...
**Implantáty** — tuhé tyčinky vpravené do tkáně, prodloužený přívod. **Lipozomy** — vezikuly z fosfolipidů, jádro i povrch hydrofilní, nitro membrány lipofilní → umístění léčiva podle povahy; umožňují **pasivní i aktivní cílení**. **Implantáty pro cílené působení** — cytostatika do CNS, **stenty potažené imunosupresivem**, IUD.
...
> **Suchá ložiska → „suché" hydrofobní přípravky** (masti).
> Akutní a intenzivní → hydrokrémy a hydrogely. Chronické a mírné → hydrofobní masti a oleokrémy.

| Forma | Definice |
|---|---|
| **Masti** (*unguenta*) | jednofázový základ, **max 20 % vody** |
| **Krémy** (*cremores*) | lipofilní + vodná fáze, **min 20 % vody** |
...
**Kdy:** když léčivo není snášeno perorálně (zvracení, nespolupráce, malé děti).

**Čípky** (*suppositoria*) — tuhé jednodávkové, při tělesné teplotě tají a rozpouštějí se. **Dospělí 2–3 g, děti 1 g.** Základy: kakaový olej (*Cacao oleum*) a **tvrdý tuk** (*Adeps solidus*).
...
**Medicinální plyny — barvy lahví** *(vděčné na doptání)*:
...
| **Difuze ve vodném prostředí** | intersticium, cytosol — Brownův pohyb po koncentračním spádu | **téměř ne** |
...
⚠️ **Klíč k pochopení:** léčiva jsou většinou slabé kyseliny nebo zásady a při každém pH existuje rovnováha mezi dvěma formami:
...
| **Bariérový endotel** — hematoencefalická, hematolikvorová, placentární, testikulární | **bez pórů, těsné spoje** | **lipofilní snadno**; polární jen přes přenašeče; makromolekuly vezikulárně |
| Cytoplazmatická membrána | < 1 nm | velmi malé hydrofilní (MH < 100) — močovina, etanol, lithium |

> Absorpční plochu tenkého střeva zvětšují **řasy, klky a mikroklky (kartáčový lem)** — proto se tam vstřebává většina perorálně podaných léčiv.
...
| Rychlostní konstanta absorpce | kₐ | 1/čas | rychlost absorpce |
...
| Rychlostní konstanta eliminace | kₑ | 1/čas | rychlost změny koncentrace |
| **Biologický poločas eliminace** | **t₁/₂** | čas | čas potřebný k poklesu koncentrace **na polovinu** |
...
Eliminační orgány odstraňují léčivo **stejně efektivně** při nízkých i vysokých koncentracích. Za **biologický poločas t₁/₂** klesne koncentrace z počáteční hodnoty **na polovinu**.
